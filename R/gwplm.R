...
gwplm <- function(SpDF, data, index, formula, bw, kernel, adaptive=F, effect=c("individual", "time", "twoways", "nested"),
                  print.effects=c("individual", "time", "twoways"), model = c("within", "random", "ht", "between", "pooling", "fd"), 
                  dMat){
  ...
  for (i in 1:n){
    ...
    for(j in 1:t){
      ...
      yHat[(i-1)*t+j] <- pmodel.response(plm.i)[(i-1)*t+j] - plm.i$residuals[(i-1)*t+j]
